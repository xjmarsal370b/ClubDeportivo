                        ...
                        <tr data-entry-id="{{ $eventOrganizer->id }}">
                            ...
                            <td>
                                @can('event_organizer_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.event-organizers.show', $eventOrganizer->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('event_organizer_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.event-organizers.edit', $eventOrganizer->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('event_organizer_delete')
                                    <form action="{{ route('admin.event-organizers.destroy', $eventOrganizer->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('event_organizer_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.event-organizers.massDestroy') }}",
