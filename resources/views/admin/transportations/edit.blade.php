...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.transportation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.transportations.update", [$transportation->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="company_name">{{ trans('cruds.transportation.fields.company_name') }}</label>
                <input class="form-control {{ $errors->has('company_name') ? 'is-invalid' : '' }}" type="text" name="company_name" id="company_name" value="{{ old('company_name', $transportation->company_name) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('transportation_type') ? 'is-invalid' : '' }}" name="transportation_type" id="transportation_type" required>
                    ...
                    @foreach(App\Models\Transportation::TRANSPORTATION_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('transportation_type', $transportation->transportation_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('dep_place') ? 'is-invalid' : '' }}" type="text" name="dep_place" id="dep_place" value="{{ old('dep_place', $transportation->dep_place) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('dep_date') ? 'is-invalid' : '' }}" type="text" name="dep_date" id="dep_date" value="{{ old('dep_date', $transportation->dep_date) }}" required>
