        ...
        <div class="col-md-12">
            @can('transportation_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.transportations.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $transportation->id }}">
                                        ...
                                        <td>
                                            @can('transportation_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.transportations.show', $transportation->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('transportation_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.transportations.edit', $transportation->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('transportation_delete')
                                                <form action="{{ route('frontend.transportations.destroy', $transportation->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('transportation_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.transportations.massDestroy') }}",
